#!/usr/bin/env python3
"""
ChartQA Experiment: Baseline vs MemCanvas.

Dataset: ChartQA (chart/document reasoning)
  - Train: 28,299
  - Val:   dataset split name is `val`
  - Test:  2,500
Metric: exact-match accuracy after normalized answer comparison

We use the validation split for faster experimentation and cleaner iteration.

Usage:
  python -u /home/cyf/codex/chartqa_experiment.py --phase prep
  CUDA_VISIBLE_DEVICES=0 python -u /home/cyf/codex/chartqa_experiment.py --phase eval
  CUDA_VISIBLE_DEVICES=0 python -u /home/cyf/codex/chartqa_experiment.py --phase all
"""
...
DATA_DIR = Path("/home/cyf/codex/chartqa_data")
OUTPUT_DIR = Path("/home/cyf/codex/chartqa_experiment")
...
def download_chartqa():
    cache_file = DATA_DIR / "chartqa_meta.pkl"
    if cache_file.exists():
        with open(cache_file, "rb") as f:
            data = pickle.load(f)
        print(f"Loaded cached ChartQA metadata: train={len(data['train'])}, val={len(data['val'])}")
        return data
    ...
    def process_split(split_data, img_dir, split_name):
        ...
        for i, item in enumerate(tqdm(split_data, desc=f"Processing {split_name}")):
            ...
            samples.append({
                ...
                "human_or_machine": item.get("human_or_machine", "unknown"),
            })
        return samples

    train_samples = process_split(train_ds, train_img_dir, "train")
    val_samples = process_split(val_ds, val_img_dir, "val")
    data = {"train": train_samples, "val": val_samples}
    with open(cache_file, "wb") as f:
        pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"Cached ChartQA metadata: train={len(train_samples)}, val={len(val_samples)}")
    return data
...
def render_chartqa_canvas(sample: dict) -> bytes:
    ...
    if img is not None:
        canvas.add_text("[Chart]", font_size=14, bold=True)
        canvas.add_image(img, max_height=420)

    canvas.add_separator()
    canvas.add_text(f"Question: {sample.get('question', '')}", font_size=17, bold=True)
    answers = sample.get("answers", [])
    if answers:
        canvas.add_text(f"✓ Answer: {answers[0]}", font_size=17)
        if len(answers) > 1:
            canvas.add_text(f"Also accepted: {', '.join(answers[1:3])}", font_size=14)
...
def build_canvases(train_data: List[dict], rebuild: bool = False):
    ...
    for i, sample in enumerate(tqdm(train_data, desc="Building canvases")):
        ...
        if out_path.exists():
            continue
        canvas_bytes = render_chartqa_canvas(sample)
...
def compute_embeddings(train_data, val_data, n_canvases):
    ...
    if not text_emb_file.exists():
        texts = [f"Question: {s['question']} Answer: {s['answers'][0] if s['answers'] else ''}" for s in train_data]
        all_emb = []
        batch_size = 64
    ...
    if not query_emb_file.exists():
        texts = [s['question'] for s in val_data]
        all_emb = []
        batch_size = 64
...
def predict_baseline(model, processor, sample):
    img = load_image(sample)
    user_text = f"Look at the chart and answer the question.\nQuestion: {sample['question']}\nAnswer concisely:"
    content = []
    images = []
    if img is not None:
...
def predict_memcanvas(model, processor, sample, retrieved):
    ...
    if retrieved:
        prompt_parts.append(
            "Below are memory canvas images from similar chart questions answered before. "
            "Each canvas shows the chart, question, and correct answer. Use them as references."
        )
...
def evaluate_condition(condition_name, model, processor, val_data, retrieval_map, output_dir):
...
def main():
    ...
    if args.phase in ("eval", "all"):
        ...
        if not args.skip_baseline:
            ...
            acc, _ = evaluate_condition("baseline", model, processor, val_data, retrieval_map, OUTPUT_DIR)
            summary["baseline"] = {"accuracy": acc}
        print("=== Evaluating MEMCANVAS ===")
        acc, _ = evaluate_condition("memcanvas", model, processor, val_data, retrieval_map, OUTPUT_DIR)
